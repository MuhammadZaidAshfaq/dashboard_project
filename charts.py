...
def chart_box(df):
    ...
    for ax, (col, ylabel) in zip(axes, [
        ...
        sns.boxplot(data=df, x='label', y=col, palette=PALETTE,
                    order=['ham', 'spam'], width=0.45, linewidth=1.4,
...
def chart_count(df):
    ...
    sns.countplot(data=df, x='label', palette=PALETTE,
                  order=['ham','spam'], edgecolor='white', ax=ax2)
...
def chart_violin(df):
    ...
    for ax, (col, ylabel) in zip(axes, [
        ...
        sns.violinplot(data=df, x='label', y=col, palette=PALETTE,
                       order=['ham','spam'], inner='quartile',
                       linewidth=1.4, ax=ax)
